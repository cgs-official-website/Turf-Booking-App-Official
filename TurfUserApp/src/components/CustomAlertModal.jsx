...
const getAlertMeta = (title = '', message = '', buttons = []) => {
  ...
  const hasDestructive = buttons.some((b) => b.style === 'destructive') ||
    /delete|remove|cancel|logout|sign out|permanent/i.test(text);

  const isSuccess = /success|booked|saved|updated|confirmed|joined|won/i.test(text);
  const isWarning = /error|failed|missing|invalid|required|warning|mismatch/i.test(text);
...
export const CustomAlertModal = () => {
  const darkMode = useSelector((s) => s.auth?.darkMode || false);
            ...
            >
              ...
              >
                {buttons.map((btn, index) => {
                  ...
                  let btnBg = colors.bgSoft || '#F3F4F6';
